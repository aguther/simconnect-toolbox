...
bool SimConnectEventInterface::connect(
...
void SimConnectEventInterface::disconnect() {
  if (isConnected) {
    // close connection
...
bool SimConnectEventInterface::readData() {
...
void SimConnectEventInterface::simConnectProcessDispatchMessage(
    SIMCONNECT_RECV *pData,
    DWORD *cbData
) {
  switch (pData->dwID) {
...
void SimConnectEventInterface::simConnectProcessEvent(
  ...
  data->set(event->uEventID, static_cast<bool>(event->dwData));
}

bool SimConnectEventInterface::prepareDataDefinition(
...
bool SimConnectEventInterface::addDataDefinition(
    HANDLE connectionHandle,
  ...
  HRESULT result = SimConnect_SubscribeToSystemEvent(
      connectionHandle,
      eventId,
      variable.name.c_str()
  );
